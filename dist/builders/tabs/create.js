...
import { addMeltEventListener, createElHelpers, disabledAttr, executeCallbacks, generateIds, getDirectionalKeys, getElemDirection, isBrowser, isHTMLElement, kbd, last, makeElement, next, omit, overridable, prev, toWritableStores, } from '../../internal/helpers/index.js';
const defaults = {
...
export function createTabs(props) {
    ...
    const getTabIds = (tabValue) => {
        const existing = tabIds.get(tabValue);
        if (existing)
            return existing;
        const generated = generateIds(['triggerId', 'contentId']);
