...
import { addMeltEventListener, createElHelpers, executeCallbacks, getElementByMeltId, isHidden, isHTMLElement, isLetter, kbd, last, makeElement, overridable, styleToString, } from '../../internal/helpers/index.js';
    ...
    const group = makeElement(name('group'), {
        ...
        returned: ([$expanded]) => {
            return (opts) => ({
                ...
                style: styleToString({
                    display: !forceVisible && !$expanded.includes(opts.id)
                        ? 'none'
                        : undefined,
                }),
            });
