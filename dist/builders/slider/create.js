...
import { addEventListener, addMeltEventListener, createElHelpers, disabledAttr, effect, executeCallbacks, generateIds, getElementByMeltId, isBrowser, isHTMLElement, kbd, makeElement, makeElementArray, omit, overridable, snapValueToStep, styleToString, toWritableStores, } from '../../internal/helpers/index.js';
...
const invalidConfigurationMessage = 'Invalid slider configuration: min, max, and every value must be finite numbers, and step must be a finite number greater than 0.';
function assertValidConfiguration(min, max, step, value) {
...
export const createSlider = (props) => {
    ...
    effect([step, min, max, value], function fixValue([$step, $min, $max, $value]) {
        assertValidConfiguration($min, $max, $step, $value);
