...
import { addMeltEventListener, createElHelpers, effect, executeCallbacks, isHTMLElement, isNumberString, kbd, makeElement, noop, omit, overridable, sleep, styleToString, toWritableStores, withGet, } from '../../internal/helpers/index.js';
import { generateIds } from '../../internal/helpers/id.js';
import { createHiddenInput } from '../hidden-input/create.js';
import { areAllSegmentsFilled, createContent, getPartFromNode, getValueFromSegments, inferGranularity, initializeSegmentValues, initSegmentStates, isAcceptableSegmentKey, isDateAndTimeSegmentObj, isDateSegmentPart, isFirstSegment, removeDescriptionElement, setDescription, syncSegmentValues, } from './_internal/helpers.js';
const defaults = {
...
export function createDateField(props) {
    ...
    const { locale, granularity, hourCycle, hideTimeZone, isDateUnavailable, disabled, readonly, readonlySegments, name: nameStore, required, minValue, maxValue, rootElement, } = options;
    ...
    const placeholderWritable = withDefaults.placeholder ??
    ...
    const resolvedHourCycle = withGet.derived([hourCycle, locale], ([$hourCycle, $locale]) => {
        if ($hourCycle)
            return $hourCycle;
        const resolved = new DateFormatter($locale, {
            hour: 'numeric',
    ...
    const label = makeElement(name('label'), {
        ...
        action: (node) => {
            const unsub = executeCallbacks(addMeltEventListener(node, 'click', () => {
                const firstSegment = getFirstSegment(ids.field.get(), rootElement.get());
                if (!firstSegment)
                    return;
    ...
    const field = makeElement(name('field'), {
        ...
        action: (_node) => {
            ...
            return {
                destroy() {
                    removeDescriptionElement(ids.description.get(), rootElement.get());
                },
            };
        },
    });
    ...
    const segment = makeElement(name('segment'), {
        ...
        returned: ([$segmentValues, $hourCycle, $placeholder, $value, $isInvalid, $disabled, $readonly, $readonlySegmentsSet, $idValues, $rootElement,]) => {
            const props = {
                segmentValues: $segmentValues,
                hourCycle: $hourCycle,
            ...
            return (part) => {
                ...
                const describedBy = hasDescription
                    ? `${$idValues.description}${$isInvalid ? ` ${$idValues.validation}` : ''}`
                    : undefined;
                return {
                    ...defaultAttrs,
                    id: $idValues[part],
    ...
    function handleDaySegmentKeydown(e) {
        ...
        if (isNumberString(e.key)) {
            ...
            if (moveToNext) {
                moveToNextSegment(e, $fieldId, rootElement.get());
            }
        }
        ...
        if (isSegmentNavigationKey(e.key)) {
            handleSegmentNavigation(e, $fieldId, rootElement.get());
        }
    }
    /*
     * -----------------------------------------------------
    ...
    function hourSegmentAttrs(props) {
        ...
        const hour = segmentValues.hour ?? placeholder.hour;
        const valueMin = hourCycle === 12 ? 1 : 0;
        const valueMax = hourCycle === 12 ? 12 : 23;
        const valueNow = hourCycle === 12 ? hour % 12 || 12 : hour;
        const valueText = isEmpty
            ? 'Empty'
    ...
    function handleHourSegmentKeydown(e) {
        ...
        const $hourCycle = resolvedHourCycle.get();
        if (e.key === kbd.ARROW_UP) {
            updateSegment('hour', (prev) => {
                ...
                const next = dateRef
                    .set({ hour: prev })
                    .cycle('hour', 1, { hourCycle: $hourCycle }).hour;
                announcer.announce(next);
                return next;
            });
            return;
        ...
        if (e.key === kbd.ARROW_DOWN) {
            updateSegment('hour', (prev) => {
                if (prev === null) {
                    const next = dateRef.cycle('hour', -1, {
                        hourCycle: $hourCycle,
                    }).hour;
                    announcer.announce(next);
                    return next;
        ...
        if (isNumberString(e.key)) {
            ...
            let moveToNext = false;
            updateSegment('hour', (prev) => {
                const maximum = $hourCycle === 12 ? 12 : 23;
                const maxStart = Math.floor(maximum / 10);
                /**
                 * If the user has left the segment, we want to reset the
                 * `prev` value so that we can start the segment over again
                 * when the user types a number.
                ...
                if (prev === null) {
                    ...
                    if (num === 0) {
                        ...
                        announcer.announce($hourCycle === 12 ? null : 0);
                        return $hourCycle === 12 ? null : 0;
                    }
                    /**
                     * If the last key was a 0, or if the first number is
    ...
    function handleDayPeriodSegmentKeydown(e) {
        ...
        if (isSegmentNavigationKey(e.key)) {
            handleSegmentNavigation(e, ids.field.get(), rootElement.get());
        }
    }
    /*
     * -----------------------------------------------------
    ...
    effect([value, rootElement], ([$value, $rootElement]) => {
        if ($value) {
        ...
        return () => removeDescriptionElement(ids.description.get(), $rootElement);
    });
    effect([value, locale, resolvedHourCycle], ([$value, _, __]) => {
