...
import { addMeltEventListener, createElHelpers, disabledAttr, executeCallbacks, isHTMLElement, makeElement, omit, overridable, toWritableStores, } from '../../internal/helpers/index.js';
...
export function createAlignmentPicker(props) {
    const withDefaults = {
        ...defaults,
        ...props,
    ...
    const item = makeElement(name('item'), {
        ...
        returned: ([$disabled, $value]) => {
            return (props) => {
                ...
                const firstEnabled = values.find((candidate) => candidate === candidate);
        ...
        action: (node) => {
            const unsub = executeCallbacks(addMeltEventListener(node, 'click', () => selectItem(node)), addMeltEventListener(node, 'keydown', (event) => {
                if (disabled.get() || node.dataset.disabled === 'true')
                    return;
                ...
                if (event.key === 'Home' || event.key === 'End') {
                    ...
                    const eligible = Array.from(rootNode.querySelectorAll(selector('item'))).filter((element) => isHTMLElement(element) && element.dataset.disabled !== 'true');
                ...
                if (event.key === 'ArrowDown')
                    rowDelta = 1;
                const eligible = new Set(Array.from(rootNode.querySelectorAll(selector('item')))
                    .filter((element) => isHTMLElement(element) && element.dataset.disabled !== 'true')
                ...
                for (let attempt = 0; attempt < 9; attempt += 1) {
                    ...
                    if (candidateValue && eligible.has(candidateValue)) {
                        target = Array.from(rootNode.querySelectorAll(selector('item'))).find((element) => isHTMLElement(element) &&
