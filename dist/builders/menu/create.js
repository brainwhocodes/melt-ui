...
import { addEventListener, addHighlight, addMeltEventListener, createElHelpers, createTypeaheadSearch, derivedVisible, disabledAttr, effect, executeCallbacks, FIRST_LAST_KEYS, generateIds, getNextFocusable, getPortalDestination, getPreviousFocusable, handleFocus, handleRovingFocus, isBrowser, isElementDisabled, isHTMLElement, isPointerInGraceArea, kbd, makeElement, noop, omit, overridable, portalAttr, removeHighlight, removeScroll, SELECTION_KEYS, sleep, styleToString, toWritableStores, } from '../../internal/helpers/index.js';
...
export function createMenuBuilder(opts) {
    ...
    const pointerMovingToSubmenu = withGet(derived([pointerDir, pointerGraceIntent], ([$pointerDir, $pointerGraceIntent]) => {
        return (e) => {
            ...
            return (isMovingTowards &&
                isPointerInGraceArea(e, $pointerGraceIntent?.area));
        };
    }));
    const { typed, handleTypeaheadSearch } = createTypeaheadSearch();
    const rootIds = toWritableStores({
        ...generateIds(menuIdParts),
        ...opts.ids,
    });
        ...
        const checkboxItem = makeElement(name('checkbox-item'), {
            ...
            returned: ([$checked, $disabled]) => {
                return {
                    ...
                    tabindex: -1,
                    'data-orientation': 'vertical',
                    'aria-checked': isIndeterminate($checked)
                        ? 'mixed'
                        : $checked
                            ? 'true'
                            : 'false',
    ...
    const createMenuRadioGroup = (args = {}) => {
        ...
        const radioItem = makeElement(name('radio-item'), {
            ...
            returned: ([$value]) => {
                return (itemProps) => {
                    const { value: itemValue, disabled } = {
                        ...radioItemDefaults,
                        ...itemProps,
                    };
                ...
                const unsubDerived = effect([subIsVisible, positioning], ([$subIsVisible, $positioning]) => {
                    ...
                    tick().then(() => {
                        ...
                        unsubPopper = usePopper(node, {
                            ...
                            options: {
                                floating: $positioning,
                                portal: isHTMLElement(parentMenuEl)
                                    ? parentMenuEl
                                    : undefined,
                                modal: null,
                                focusTrap: null,
                ...
                const unsubEvents = executeCallbacks(addMeltEventListener(node, 'keydown', (e) => {
                    ...
                    if (isUsingKeyboard.get()) {
                        ...
                        if (!menuEl.contains(relatedTarget) &&
                            relatedTarget !== $subActiveTrigger) {
                            subOpen.set(false);
                        }
                    }
                }));
    ...
    safeOnMount(() => {
        ...
        const handleKeyDown = () => {
            ...
            unsubs.push(executeCallbacks(addEventListener(document, 'pointerdown', handlePointer, {
                capture: true,
                once: true,
    ...
    effect([rootOpen], ([$rootOpen]) => {
        ...
        handleFocus({
            prop: closeFocus.get(),
            defaultEl: rootActiveTrigger.get(),
        });
    }, { skipFirstRun: true });
...
export function handleMenuNavigation(e, loop) {
    ...
    switch (e.key) {
        case kbd.ARROW_DOWN:
            if (loop) {
                nextIndex =
                    currentIndex < candidateNodes.length - 1 ? currentIndex + 1 : 0;
            }
            else {
                nextIndex =
        ...
        case kbd.ARROW_UP:
            if (loop) {
                nextIndex =
                    currentIndex > 0 ? currentIndex - 1 : candidateNodes.length - 1;
            }
            else {
                nextIndex =
                    currentIndex < 0
