import { getLocalTimeZone, getMinimumMonthInYear, isSameDay, isSameMonth, isToday, } from '@internationalized/date';
import { tick } from 'svelte';
import { derived, writable } from 'svelte/store';
import { createFormatter, createMonths, dateStore, getAnnouncer, getDefaultDate, getSelectableCells, hasTime, isAfter, isBefore, isCalendarCell, parseStringToDateValue, setPlaceholderToNodeValue, toDate, } from '../../internal/helpers/date/index.js';
import { addMeltEventListener, createElHelpers, effect, executeCallbacks, generateIds, isBrowser, isHTMLElement, isValidIndex, kbd, makeElement, omit, overridable, styleToString, toWritableStores, withGet, } from '../../internal/helpers/index.js';
...
export function createCalendar(props) {
        ...
        returned: ([$isDateSelected, $isDateDisabled, $isDateUnavailable, $isOutsideVisibleMonths, $placeholder,]) => {
            ...
            return (cellValue, monthValue) => {
                ...
                return {
                    ...
                    'data-outside-visible-months': isOutsideVisibleMonths
                        ? ''
                        : undefined,
                    'data-focused': isFocusedDate ? '' : undefined,
                    tabindex: isFocusedDate
                        ? 0
    ...
    function setMonth(month) {
        const $placeholder = placeholder.get();
        const minimum = getMinimumMonthInYear($placeholder);
        const maximum = minimum + $placeholder.calendar.getMonthsInYear($placeholder) - 1;
        if (!Number.isInteger(month) || month < minimum || month > maximum) {
            throw new Error(`Month must be an integer between ${minimum} and ${maximum}.`);
        }
