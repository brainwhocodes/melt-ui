...
export function normalizeColor(value) {
    if (!Number.isFinite(value.hue) ||
        ...
        throw new Error('Invalid color picker value: hue, saturation, value, and alpha must be finite numbers.');
    }
    return {
...
export function hsvaToRgbFloat(color) {
...
export function hsvaToRgb(color) {
...
export function hsvaToHsl(color) {
...
export function rgbToOklch({ red, green, blue }) {
    ...
    if (hue < 0)
        hue += 360;
    return {
        lightness: clamp((0.2104542553 * l + 0.793617785 * m - 0.0040720468 * s) * 100, 0, 100),
...
export function hsvaToOklch(color) {
    return rgbToOklch(hsvaToRgbFloat(color));
}
export function oklchToHsva(oklch, alpha, hue = 0) {
    return rgbToHsva(oklchToRgb(oklch), alpha, hue);
}
export function rgbToHsva(rgb, alpha, hue = 0) {
...
export function formatSerializedHex(color) {
    return `${formatHex(color)}${toHex(color.alpha * 255)}`.toUpperCase();
}
export function parseHex(input, alphaFallback) {
...
export function formatChannel(color, channel, format = 'hsv') {
    ...
    switch (channel) {
        ...
        case 'hue':
            if (oklch)
                return oklch.hue.toFixed(1);
            return String(Math.round(format === 'hsl' ? hsl.hue : color.hue));
        case 'saturation':
            return String(Math.round(format === 'hsl' ? hsl.saturation : color.saturation));
        case 'lightness':
            return oklch
...
export function getChannelBounds(channel) {
...
export function channelValue(color, channel, input, format = 'hsv') {
    if (format === 'hsl' &&
    ...
    if (channel === 'hue')
        next.hue = input;
    if (channel === 'saturation')
        next.saturation = input;
    if (channel === 'value')
        next.value = input;
    if (channel === 'alpha')
        next.alpha = input / 100;
    if (channel === 'red' || channel === 'green' || channel === 'blue') {
        const rgb = hsvaToRgb(color);
        rgb[channel] = input;
...
const cssAlphaPart = (alpha) => alpha >= 1 ? '' : ` / ${Math.round(alpha * 1000) / 1000}`;
export function serializeColor(color, format) {
    const rgb = hsvaToRgb(color);
    const hsl = hsvaToHsl(color);
    if (format === 'hex')
...
function parseComponents(body) {
    ...
    if (alphaPart !== undefined && alpha === null)
        return null;
    return { parts: parts, alpha };
}
const alphaToken = (token, fallback) => token === null
    ? fallback
    : clamp(token.percent ? token.value / 100 : token.value, 0, 1);
const rgbToken = (token) => clamp(token.percent ? (token.value / 100) * 255 : token.value, 0, 255);
const percentToken = (token) => clamp(token.value, 0, 100);
const hueToken = (token) => clamp(token.value, 0, 360);
const oklchLightnessToken = (token) => clamp(token.percent ? token.value : token.value * 100, 0, 100);
const oklchChromaToken = (token) => clamp(token.percent ? (token.value / 100) * 0.4 : token.value, 0, 0.5);
export function parseColorString(input, format, alphaFallback) {
    ...
    if (!parsed || parsed.parts.length !== 3)
        return null;
...
export function formatDisplayValue(color, format) {
