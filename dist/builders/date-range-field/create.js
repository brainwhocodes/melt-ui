...
import { areAllDaysBetweenValid, dateStore, getAnnouncer, getDefaultDate, getFirstSegment, isBeforeOrSame, } from '../../internal/helpers/date/index.js';
import { addMeltEventListener, createElHelpers, effect, executeCallbacks, makeElement, omit, overridable, sleep, styleToString, toWritableStores, } from '../../internal/helpers/index.js';
...
export function createDateRangeField(props) {
    ...
    const label = makeElement(name('label'), {
        ...
        action: (node) => {
            const unsub = executeCallbacks(addMeltEventListener(node, 'click', () => {
                const firstSegment = getFirstSegment(ids.field.get(), options.rootElement.get());
                if (!firstSegment)
                    return;
    ...
    const field = makeElement(name('field'), {
        ...
        action: (_node) => {
            ...
            return {
                destroy() {
                    removeDescriptionElement(ids.description.get(), options.rootElement.get());
                },
            };
        },
    });
