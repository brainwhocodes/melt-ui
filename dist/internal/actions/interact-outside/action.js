import { getOwnerDocument } from '../../helpers/elements.js';
import { addEventListener, debounce, executeCallbacks, noop, } from '../../helpers/index.js';
const layers = new Set();
function captureEventData(event) {
    const path = event.composedPath();
    const originalTarget = path.find((target) => target instanceof Node) ??
...
export const useInteractOutside = ((node, config = {}) => {
    ...
    function update(config) {
         ...
         * allowing a comprehensive check for intercepted events thereafter.
         */
        const onPointerUpDebounced = debounce((e, computedEventData) => {
            const eventData = computedEventData ?? captureEventData(e);
    ...
    function shouldTriggerInteractOutside(e, computedEventData) {
        if (isPointerDown &&
            !isPointerDownInside &&
...
function isValidEvent(e, node, computedEventData) {
    if ('button' in e && e.button > 0)
        return false;
    if (!computedEventData.originalTarget)
        return false;
