...
export function provideOverlayContext(key, context) {
    setContext(key, context);
    return context;
}
export function useOverlayContext(key, component) {
    const context = getContext(key);
    if (!context) {
        throw new Error(`${component} must be used inside its matching root component.`);
    }
    return context;
}
...
export function focusInitial(container, preferredSelector) {
    if (typeof window === 'undefined')
        return;
    queueMicrotask(() => {
        ...
        const target = preferred ?? autofocus ?? getFocusableElements(container)[0] ?? container;
        if (target === container && !container.hasAttribute('tabindex'))
            container.tabIndex = -1;
        target.focus({ preventScroll: true });
    });
}
export function containFocus(container) {
...
export function lockBodyScroll() {
    ...
    return () => {
        ...
        if (bodyLockCount !== 0 || !bodySnapshot)
            return;
        const body = document.body;
...
let overlayId = 0;
export function createOverlayId(prefix) {
    overlayId += 1;
