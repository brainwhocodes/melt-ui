...
const formats: ColorPickerFormat[] = ['hex', 'rgb', 'hsl', 'hsv', 'oklch'];
const channelSets: Record<ColorPickerFormat, readonly ColorPickerChannel[]> = {
	hex: ['hex', 'alpha'],
...
const cssAlpha = (alpha: number) => String(Math.round(alpha * 1000) / 1000);

function cssColor(color: ColorPickerValue) {
	const { red, green, blue } = hsvaToRgb(color);
	return `rgb(${red} ${green} ${blue} / ${cssAlpha(color.alpha)})`;
}

function cssOpaqueColor(color: ColorPickerValue) {
	const { red, green, blue } = hsvaToRgb(color);
	return `rgb(${red} ${green} ${blue})`;
}

function cssHueColor(color: ColorPickerValue) {
...
function getEyeDropperCtor(): EyeDropperConstructor | null {
	...
	const ctor: unknown = window.EyeDropper;
...
export function createColorPicker(props?: CreateColorPickerProps) {
	...
	const previousValue =
		withDefaults.previousValue ?? writable(normalizeColor(value.get()));
	const formatWritable =
		withDefaults.format ?? writable(withDefaults.defaultFormat);
	const format = overridable(formatWritable, withDefaults.onFormatChange);
	const active = writable<ActiveControl>(null);
	const activeAreaAxis = writable<AreaAxis>(null);
	const colorInputDraft = writable<string | null>(null);
	const channelDrafts = writable<Record<string, string>>({});

	effect(value, ($value) => {
		const normalized = normalizeColor($value);
		if (!equalColor(normalized, $value)) value.set(normalized);
	});
	effect(format, ($format) => {
		if (!formats.includes($format)) format.set('hex');
	});

	const updateValue = (next: ColorPickerValue) => {
		if (disabled.get() || readonlyOption.get()) return false;
		const normalized = normalizeColor(next);
		if (equalColor(normalized, value.get())) return false;
	...
	const area = makeElement(name('area'), {
		...
		action: (
			node: HTMLElement,
		): MeltActionReturn<ColorPickerEvents['area']> => {
			let pointerId: number | null = null;
			let changed = false;
			const apply = (event: PointerEvent) => {
				if (pointerId !== (event.pointerId ?? 0)) return;
				changed =
					updateValue({
						...value.get(),
						...updateAreaPosition(
			...
			return {
				destroy: executeCallbacks(
					addMeltEventListener(node, 'pointerdown', (event) => {
						if (
							disabled.get() ||
							readonlyOption.get() ||
					...
					addMeltEventListener(node, 'pointermove', apply),
					addMeltEventListener(node, 'pointerup', finish),
					addMeltEventListener(node, 'pointercancel', finish),
				),
			};
		},
	});
	...
	const areaInput = (axis: 'x' | 'y', part: 'area-x-input' | 'area-y-input') =>
		makeElement(name(part), {
			...
			action: (node: HTMLInputElement) => {
				const focusAxis = (nextAxis: 'x' | 'y') => {
					const rootNode = node.closest(selector());
					const next = rootNode?.querySelector<HTMLInputElement>(
						nextAxis === 'x'
				...
				const keyboard = (event: KeyboardEvent) => {
					...
					if (readonlyOption.get()) {
						if (mutationKey) event.preventDefault();
						return;
					}
					if (disabled.get() || !mutationKey) return;
					const horizontal =
					...
					if (event.key === 'PageUp' || event.key === 'PageDown') {
						const current =
							axis === 'x' ? value.get().saturation : value.get().value;
				...
				return {
					destroy: executeCallbacks(
						...
						addMeltEventListener(node, 'keydown', keyboard),
						addMeltEventListener(node, 'input', input),
					),
				};
			},
		});
	...
	const track = (kind: 'hue' | 'alpha', part: 'hue-track' | 'alpha-track') =>
		makeElement(name(part), {
			...
			action: (node: HTMLElement) => {
				...
				const apply = (event: PointerEvent) => {
					if (pointerId !== (event.pointerId ?? 0)) return;
					const next = updateTrackPosition(
						node,
						event.clientX ?? 0,
						event.clientY ?? 0,
						dir.get(),
	...
	const thumb = (kind: 'hue' | 'alpha', part: 'hue-thumb' | 'alpha-thumb') =>
		makeElement(name(part), {
	...
	const sliderInput = (
		...
		makeElement(name(part), {
			...
			action: (node: HTMLInputElement) => {
				const currentValue = () =>
					kind === 'hue' ? value.get().hue : value.get().alpha * 100;
				const keyboard = (event: KeyboardEvent) => {
					const mutationKey = [
						'ArrowLeft',
						'ArrowRight',
				...
				return {
					destroy: executeCallbacks(
						addMeltEventListener(node, 'focus', () => active.set(kind)),
						addMeltEventListener(node, 'keydown', keyboard),
						addMeltEventListener(node, 'input', input),
					),
				};
	...
	const channelInput = makeElement(name('channel-input'), {
		...
		action: (node: HTMLInputElement) => {
			let currentChannel: ColorPickerChannel | null = null;
			let currentFormat: ColorPickerFormat = format.get();
			let sessionAlpha = value.get().alpha;
			const draftKey = () =>
				currentChannel ? `${currentFormat}:${currentChannel}` : null;
			const setDraft = () => {
				const key = draftKey();
				if (!key) return;
			...
			const parseAndUpdate = (channel: ColorPickerChannel, raw: string) => {
				if (channel === 'hex') {
			...
			const normalizeField = (channel: ColorPickerChannel) => {
				node.value = formatChannel(value.get(), channel, currentFormat);
				delete node.dataset.invalid;
			};
			const input = () => {
				if (!currentChannel || disabled.get() || readonlyOption.get()) return;
				setDraft();
				const valid = parseAndUpdate(currentChannel, node.value);
				if (valid) delete node.dataset.invalid;
				else node.dataset.invalid = 'true';
			};
			const blur = () => {
				if (!currentChannel) return;
				if (node.dataset.invalid === 'true') normalizeField(currentChannel);
				else {
			...
			return {
				destroy: executeCallbacks(
					addMeltEventListener(node, 'focus', () => {
						currentChannel = node.dataset.channel as ColorPickerChannel;
						currentFormat =
							(node.dataset.format as ColorPickerFormat | undefined) ??
							format.get();
						sessionAlpha = value.get().alpha;
						setDraft();
					}),
					addMeltEventListener(node, 'input', input),
					addMeltEventListener(node, 'change', input),
					addMeltEventListener(node, 'blur', blur),
	...
	const formatSelect = makeElement(name('format-select'), {
		...
		action: (node: HTMLSelectElement) => ({
			destroy: addMeltEventListener(node, 'change', () => {
				if (disabled.get()) return;
				const next = node.value as ColorPickerFormat;
	...
	const previousSwatch = makeElement(name('previous-swatch'), {
		...
		action: (node: HTMLButtonElement) => ({
			destroy: addMeltEventListener(node, 'click', () => {
				if (disabled.get() || readonlyOption.get()) return;
				if (updateValue(normalizeColor(get(previousValue)))) commit();
			}),
		}),
	});
	...
	const copyButton = makeElement(name('copy-button'), {
		stores: [disabled],
		returned: ([$disabled]) => {
			return ({ format: copyFormat }: ColorPickerCopyButtonProps = {}) =>
				({
					type: 'button',
			...
			destroy: addMeltEventListener(node, 'click', () => {
				...
				void new EyeDropperCtor()
					...
					.then((result) => {
						...
						if (parsed && updateValue(parsed)) commit();
					})
					.catch(() => undefined);
			}),
		}),
	...
	const colorInput = makeElement(name('color-input'), {
		...
		action: (node: HTMLInputElement) => {
			...
			const input = () => {
				if (disabled.get() || readonlyOption.get()) return;
				if (get(colorInputDraft) === null) return;
				colorInputDraft.set(node.value);
				const parsed = parseColorString(node.value, format.get(), sessionAlpha);
				if (parsed && updateValue(parsed)) delete node.dataset.invalid;
				else if (!parsed) node.dataset.invalid = 'true';
			};
			const blur = () => {
				if (get(colorInputDraft) === null) return;
				if (node.dataset.invalid === 'true') normalizeField();
				else {
					input();
					normalizeField();
					commit();
	...
	const serializedState = derived([value, format], ([$value, $format]) =>
		serializeColor($value, $format),
	);
